...
public static class LiveToolCall {
  ...
  public static async Task RunConversation(bool vertexAI = false) {
    Types.LiveConnectConfig config = new Types.LiveConnectConfig {
          Tools = new List<Types.Tool> { new Types.Tool {
            FunctionDeclarations =
                new List<Types.FunctionDeclaration> { new Types.FunctionDeclaration {
      ...
      while (!cancellationToken.IsCancellationRequested) {
        ...
        if (serverMessage?.ServerContent?.ModelTurn?.Parts != null) {
          foreach (var part in serverMessage.ServerContent.ModelTurn.Parts)
          {
            ...
            {
              Console.WriteLine("executable code: " + part.ExecutableCode.Code);
            }
          }
        } else if (serverMessage?.ToolCall?.FunctionCalls != null) { 
          foreach (var functionCall in serverMessage.ToolCall.FunctionCalls) {
            Console.WriteLine($"Tool call: {functionCall.Name} with arguments {functionCall.Args.ToString()}");
          }
